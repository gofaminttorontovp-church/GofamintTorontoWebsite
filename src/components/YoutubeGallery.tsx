...
export default function YoutubeGallery() {
  return (
    <section id="videos" className="overflow-x-clip bg-white px-6 py-20 md:px-8 md:py-24">
      <div className="mx-auto flex max-w-6xl flex-col gap-12 md:gap-16">
        <header className="flex flex-col items-center gap-3 text-center">
          ...
          <a
            ...
            /* The colour is set here rather than with a utility class: the
               unlayered `a { color: inherit }` in globals.css outranks
               anything Tailwind puts in its utilities layer. */
            className="btn mt-2 gap-2.5 !text-[16px] font-normal"
            style={{ background: "#1d1d1f", color: "#ffffff" }}
          >
            {/* The mark takes YouTube's own red on a near-black pill, which is
                how the footer already sets it. */}
